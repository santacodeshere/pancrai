    ...
    if report_html:
        ...
        with col1:
            st.download_button(
                ...
                f"PancrAI_{patient_info.get('name','patient')}"
...
def page_dashboard():
    ...
    stats = api_get("/dashboard/stats") or {
        ...
        "avg_confidence": 0, "detection_rate": 0,
    ...
    tab1, tab2, tab3 = st.tabs(
        ["📊 Performance", "📈 Calibration", "🗂️ Confusion Matrix"])
    ...
    with tab1:
        ...
        with c1:
            ...
            if dist:
                ...
                st.markdown(
                    '<div class="info-box">Analyze scans to populate chart.</div>',
                    unsafe_allow_html=True)
        with c2:
            import pandas as pd
    ...
    badges = [
        ...
        "Grad-CAM Explainability", "TNM Staging", "RECIST Measurement",
        "CA 19-9 Integration", "Differential Diagnosis", "SHAP Importance",
        "Survival Curves", "3D Visualization", "Gemini AI Reports",
        "Groq Chat", "PDF Export", "DICOM Metadata",
...
def page_new_scan():
    ...
    with col_opt1:
        use_ensemble = st.toggle(
            "🤝 Ensemble Inference (TransUNet + LightUNet)", value=False)
    with col_opt2:
    ...
    if not uploaded_file:
        st.markdown(
            '<div class="info-box">📎 Upload a scan to begin.</div>',
            unsafe_allow_html=True)
        if (st.session_state.get("last_result")
                and st.session_state.get("last_patient")):
        ...
        <div style="background:#161B22;border:1px solid #21262D;
            ...
            <div style="margin-top:10px">
                <div><span style="color:#8B949E">Name:</span>
                     ...
                     {uploaded_file.name}</span></div>
                <div><span style="color:#8B949E">Size:</span>
                     <span style="color:#E6EDF3;font-family:monospace">
                     {uploaded_file.size/1024:.1f} KB</span></div>
                <div><span style="color:#8B949E">Mode:</span>
                     <span style="color:#3FB950">
                     {"Ensemble" if use_ensemble
                       else "TTA 8x" if use_tta
                       else "Standard"}</span></div>
                <div><span style="color:#8B949E">CA 19-9:</span>
                     <span style="color:#E6EDF3">
                     {f"{ca199_val:.0f} U/mL" if ca199_val
                       else "Not provided"}</span></div>
            </div>
        </div>""", unsafe_allow_html=True)
    ...
    if st.button("🚀 Analyze Scan", type="primary", key="btn_analyze"):
        ...
        with st.spinner(
            f"🔄 Running {'Ensemble' if use_ensemble else 'TTA'} analysis..."
        ):
...
def _render_dicom_metadata(file_bytes):
    ...
    with st.expander("🏥 DICOM Metadata", expanded=False):
        c1, c2, c3 = st.columns(3)
        for col, section_key, title in [
            (c1, "patient",     "Patient"),
            (c2, "study",       "Study"),
            (c3, "acquisition", "Acquisition"),
        ]:
            with col:
                st.markdown(f"**{title}**")
                for k, v in meta.get(section_key, {}).items():
                    if v != "N/A":
                        st.markdown(
                            f'<div class="dicom-field">'
                            f'<span class="dicom-label">'
...
def _render_results_panel(result, patient_info,
                           ca199_val=None, symptoms_flags=None):
    st.markdown("---")
    ...
    unc_color   = ("#4CAF50" if unc < 30
                   else "#FF9800" if unc < 60
                   else "#F44336")
    ...
    if result.get("ensemble_enabled"):
        ...
        st.markdown(
            ...
            f' — TransUNet (70%) + LightUNet (30%) | '
            f'Model Agreement: {agr:.1f}%</div>',
            unsafe_allow_html=True)
    elif result.get("tta_enabled"):
        st.markdown(
            '<div class="success-box">🔁 <strong>TTA Active</strong>'
            ' — 8x augmented predictions averaged</div>',
            unsafe_allow_html=True)
    ...
    with c1:
        st.markdown(f"""<div class="metric-card">
            ...
            <div style="font-size:1.1rem;font-weight:700;color:{tc_color};
                 margin-top:8px">{tumor_class}</div>
        </div>""", unsafe_allow_html=True)
    with c2:
        st.markdown(f"""<div class="metric-card">
    ...
    if unc_det.get("high_uncertainty_warning"):
        st.markdown(
            f'<div class="warning-box">⚠️ '
            f'{unc_det.get("warning_message","High uncertainty.")}</div>',
            unsafe_allow_html=True)
    ...
    with tabs[1]:
        ...
        if gc_b64 or gcpp_b64:
            st.markdown("**Grad-CAM Attention Maps**")
            col_gc, col_gcpp = st.columns(2)
            with col_gc:
                st.markdown("*Grad-CAM*")
                if gc_b64:
                    pil_gc = b64_to_image(gc_b64)
                    if pil_gc:
                        st.image(pil_gc, use_container_width=True)
                else:
                    st.info("Grad-CAM not available")
            with col_gcpp:
                st.markdown("*Grad-CAM++*")
                if gcpp_b64:
                    pil_gcpp = b64_to_image(gcpp_b64)
                    if pil_gcpp:
                        st.image(pil_gcpp, use_container_width=True)
                else:
                    st.info("Grad-CAM++ not available")
            st.caption("Red = high model attention | Blue = low attention")
        else:
            st.markdown("**Grad-CAM Attention Map**")
            st.info("Grad-CAM not available for this scan")
        ...
        if measurements:
            ...
            for col, (label, val) in zip(mc, [
                ...
                ("Centroid",
                 f"({measurements.get('centroid_x',0):.0f}, "
                 f"{measurements.get('centroid_y',0):.0f}) px"),
                ("Bounding Box",
                 f"{measurements.get('bbox_w',0)} x "
                 f"{measurements.get('bbox_h',0)} px"),
                ("Aspect Ratio",
                 f"{measurements.get('aspect_ratio',0):.3f}"),
            ]):
                col.markdown(f"""<div class="metric-card">
                    <div class="label">{label}</div>
        ...
        cd, ci2 = st.columns(2)
        with cd:
            st.metric("Dice Score",
                      f"{dice:.4f}" if dice > 0 else "N/A")
        with ci2:
            st.metric("IoU Score",
                      f"{result.get('iou_score',0):.4f}"
    ...
    with tabs[2]:
        ...
        if radiomics:
            ...
            with c1:
                ...
                st.plotly_chart(
                    create_radiomics_radar_chart(radiomics),
                    use_container_width=True)
            with c2:
                if shap:
            ...
            if interp:
                ...
                for k, v in interp.items():
                    color = ("#F44336"
                             if "irregular" in v.lower() or "malignant" in v.lower()
    ...
    with tabs[3]:
        ...
        if staging:
            ...
            with col1:
                ...
                <div style="background:#161B22;border:1px solid #21262D;
                    ...
                    <div style="font-size:2rem;font-weight:700;color:{t_color};
                         text-align:center">{ts.get('t_stage','N/A')}</div>
                    <div style="color:#8B949E;text-align:center;margin-top:4px">
                         {ts.get('description','')}</div>
                    <div style="margin-top:12px">
                        <div><span style="color:#8B949E">Stage:</span>
                             <span style="color:#E6EDF3;font-weight:700">
                             {os_.get('overall_stage','N/A')}</span></div>
                        <div><span style="color:#8B949E">TNM:</span>
                             <span style="color:#E6EDF3">
                             {os_.get('tnm','N/A')}</span></div>
                        <div><span style="color:#8B949E">5-Year Survival:</span>
                             <span style="color:#E6EDF3">
                             {os_.get('five_year_survival','N/A')}</span></div>
                    </div>
                </div>""", unsafe_allow_html=True)
            with col2:
                ...
                <div style="background:#161B22;border:1px solid #21262D;
                     border-radius:8px;padding:16px">
                    <div style="font-size:1.1rem;font-weight:700;color:{res_color}">
                         {res.get('status','N/A')}</div>
                    <div style="color:#8B949E;font-size:0.85rem;margin-top:8px">
                         {res.get('description','')}</div>
                    <div style="margin-top:12px;color:#E6EDF3;font-size:0.85rem">
                        <strong>Procedure:</strong><br>
                        {res.get('procedure','MDT review required')}
                    </div>
                </div>""", unsafe_allow_html=True)
            ...
            if recs:
                ...
                for rec in recs:
                    color = ("#F44336"
                             if any(x in rec for x in ["🚨", "🔴", "⚠"])
                             else "#FF9800"
                             if any(x in rec for x in ["⚕️", "🔬"])
                             else "#4CAF50")
                    st.markdown(
                        f'<div style="background:#161B22;border-left:3px solid {color};'
                        f'padding:8px 12px;margin:4px 0;border-radius:0 4px 4px 0;'
    ...
    with tabs[4]:
        ...
        if differential:
            ...
            for i, dx in enumerate(differential[:4]):
                ...
                with st.expander(
                    f"#{i+1} {dx['diagnosis']} — {prob}", expanded=(i == 0)
                ):
                    c1, c2 = st.columns(2)
                    with c1:
    ...
    with tabs[6]:
        ...
        if staging:
            ...
            stage = staging.get("overall_stage", {}).get(
                "overall_stage", "Stage IIA")
            fig = create_survival_curve(
                tumor_class, stage, patient_info.get("age"))
            st.plotly_chart(fig, use_container_width=True)
            surv = staging.get("overall_stage", {}).get(
                "five_year_survival", "N/A")
            prog = staging.get("overall_stage", {}).get("prognosis", "N/A")
            st.markdown(
                f'<div class="info-box">📊 '
                f'<strong>Estimated 5-Year Survival:</strong> {surv} — {prog}</div>',
                unsafe_allow_html=True)
        else:
        ...
        if ca199_val is not None and staging:
            ...
            for comp, data in combined["components"].items():
                if isinstance(data, dict):
                    ...
                    color  = ("#F44336" if pct > 0.7
                              else "#FF9800" if pct > 0.4
                              else "#4CAF50")
                    st.markdown(f"""
                    <div style="background:#161B22;border:1px solid #21262D;
            ...
            if combined.get("ca199_clinical_note"):
                st.markdown(
                    ...
                    f'{combined["ca199_clinical_note"]}</div>',
                    unsafe_allow_html=True)
        else:
            st.markdown(
    ...
    with tabs[8]:
        measurements = result.get("measurements")
        has_tumor    = (measurements is not None
                        and measurements.get("area_cm2", 0) > 0.01)
        if has_tumor:
            import cv2
            ...
            cv2.ellipse(fake_mask, (cx, cy), (bw // 2, bh // 2),
                        0, 0, 360, 255, -1)
            fig = create_3d_tumor_surface(
                fake_mask,
                title=(f"3D Tumor — {tumor_class} | "
                       f"{measurements.get('area_cm2',0):.3f} cm²"),
                color="#FF4444",
            )
            st.plotly_chart(fig, use_container_width=True)
            ...
            c2.metric("Cross-section Area",
                      f"{measurements.get('area_cm2',0):.3f} cm²")
            ar = measurements.get("aspect_ratio", 1)
            c3.metric("Shape",
                      "Round" if ar < 1.3
                      else "Oval" if ar < 2.0
                      else "Elongated")
            st.caption(
                "3D ellipsoid approximation. "
                "Upload NIfTI for true volumetric rendering.")
        else:
    ...
    if not st.session_state.get("last_report"):
        st.markdown(
            '<div class="info-box">Generate a full structured clinical '
            'report with Gemini AI.</div>',
            unsafe_allow_html=True)
...
def page_chat():
    ...
    if result:
        ...
        risk     = result.get("risk_level", "Unknown")
        staging  = result.get("staging", {})
        stage    = (staging.get("overall_stage", {}).get("overall_stage", "N/A")
                    if staging else "N/A")
        ...
        st.markdown(
            '<div class="warning-box">⚠️ Analyze a scan first for '
            'context-aware responses.</div>',
            unsafe_allow_html=True)
    ...
    if not st.session_state.chat_history:
        st.markdown("""<div style="text-align:center;color:#8B949E;padding:40px 0">
            ...
            <div style="margin-top:8px">Ask anything about findings, staging,
                 treatment, or guidelines.</div>
...
def page_comparison():
    st.markdown('<div class="section-header">📈 Longitudinal Comparison</div>',
                unsafe_allow_html=True)
    st.markdown(
        '<div class="info-box">Upload two scans from different time points.</div>',
        unsafe_allow_html=True)
    st.markdown("<br>", unsafe_allow_html=True)
    ...
    if st.button("🔍 Compare Scans", type="primary"):
        ...
        with c1:
            ...
            img1 = b64_to_image(r1["seg"].get("overlay_b64", ""))
            if img1:
                st.image(img1, use_container_width=True)
            tc1_color = CLASS_COLORS.get(r1["class"], "#9E9E9E")
            st.markdown(
                f'<span style="color:{tc1_color};font-weight:700">'
                f'{r1["class"]}</span> — {r1["confidence"]*100:.1f}%',
                unsafe_allow_html=True)
            if r1.get("staging"):
                st.caption(
                    f"Stage: {r1['staging'].get('overall_stage',{}).get('overall_stage','N/A')}")
            if r1.get("recist", {}).get("available"):
                st.caption(f"LD: {r1['recist']['longest_diameter_mm']}mm")
        with c2:
            st.markdown(f"**Scan 2 — {date2}**")
            img2 = b64_to_image(r2["seg"].get("overlay_b64", ""))
            if img2:
                st.image(img2, use_container_width=True)
            tc2_color = CLASS_COLORS.get(r2["class"], "#9E9E9E")
            st.markdown(
                f'<span style="color:{tc2_color};font-weight:700">'
                f'{r2["class"]}</span> — {r2["confidence"]*100:.1f}%',
                unsafe_allow_html=True)
            if r2.get("staging"):
                st.caption(
                    f"Stage: {r2['staging'].get('overall_stage',{}).get('overall_stage','N/A')}")
        ...
        direction = ("grown" if change > 5
                     else "shrunk" if change < -5 else "stable")
        box = ("warning-box" if change > 5
               else "success-box" if change < -5 else "info-box")
        st.markdown(
...
def page_records():
    ...
    with tab1:
        ...
        if not patients:
            st.markdown(
                '<div class="info-box">No patients found.</div>',
                unsafe_allow_html=True)
            return
        import pandas as pd
        df = pd.DataFrame([{
    ...
    with tab2:
        ...
        if submitted:
            if not name:
                ...
                if res:
                    st.success(
                        f"✅ Patient '{name}' created — ID: {res['id']}")
